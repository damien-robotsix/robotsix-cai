...
|---|---|---|---|---|---|
| `cai-analyze` | Analyze parsed transcript signals and raise auto-improve findings; writes findings to findings.json | Read, Grep, Glob, Skill, Write | sonnet | Scheduled (cron) | Read-only |
...
| `cai-audit` | Audit issue queue and PRs for lifecycle state-machine inconsistencies; writes findings to findings.json | Read, Grep, Glob, Write | sonnet | Scheduled (cron) | Read-only |
| `cai-check-workflows` | Analyze recent GitHub Actions workflow failures and write structured findings to findings.json for new, unreported failures | Read, Grep, Glob, Write | haiku | Scheduled (cron) | Read-only |
...
| `cai-code-audit` | Read-only source tree audit for inconsistencies, dead code, and missing cross-file references; writes findings to findings.json | Read, Grep, Glob, Write | sonnet | Scheduled (weekly, cron) | Worktree |
...
| `cai-external-scout` | Weekly scout for open-source libraries that could replace in-house plumbing; writes one adoption proposal per run to findings.json | Read, Grep, Glob, WebSearch, WebFetch, Write | opus | Scheduled (weekly, cron) | Worktree |
...
| `cai-propose` | Weekly creative agent that proposes ambitious improvements | Read, Grep, Glob | sonnet | Scheduled (weekly, cron) | Worktree |
...
| `cai-update-check` | Periodic Claude Code release checker — writes findings to findings.json for new versions, deprecations, and best-practice changes | Read, Grep, Glob, Write | sonnet | Scheduled (cron) | Worktree |
