...
| `REFINED` | [`handle_plan`](https://github.com/damien-robotsix/robotsix-cai/blob/main/cai_lib/actions/plan.py) | `cai-plan` ×2 (serial) |
| `PLANNING` | [`handle_plan`](https://github.com/damien-robotsix/robotsix-cai/blob/main/cai_lib/actions/plan.py) (resume) | `cai-plan` |
...
| `cai-git` | INTERNAL — Lightweight subagent that executes git operations on behalf of other agents | Bash | haiku | Helper (spawned by other agents via Agent tool) | Worktree |
...
| `cai-merge` | INTERNAL — Assess whether a PR correctly implements its linked issue and emit a merge verdict; `docs/**`, `CODEBASE_INDEX.md`, and files cited in pre-merge review findings are automatically exempt from scope checks | Read | opus | PR state APPROVED | Inline-only |
| `cai-memorize` | INTERNAL — Post-solved memory curator — decides whether a solved issue settled a cross-cutting design decision worth persisting to the shared agent memory pool | Read, Write, Edit, Glob | sonnet | Inline, invoked by handle_confirm after solved | Inline-only |
...
| `cai-rebase` | INTERNAL — Lightweight rebase conflict resolution for PRs with no unaddressed review comments | Read, Edit, Write, Grep, Glob, Agent | haiku | PR state REBASING (and conflict-only REVISION_PENDING) | Worktree |
...
| `cai-rescue` | Autonomous rescue agent — decides whether a `:human-needed` issue or `:pr-human-needed` PR divert can be resumed without admin input (including a one-shot Opus-escalation of the implement phase, issue-side only), and optionally proposes a prevention finding | Read, Grep, Glob | sonnet | Scheduled (on-demand via `cai rescue`) | Worktree |
| `cai-review-docs` | Pre-merge documentation review — checks whether PR changes require `/docs` updates, directly fixes stale documentation, and raises out-of-scope issues for documentation gaps outside the PR's authorized scope | Read, Grep, Glob, Edit, Write | haiku | PR state REVIEWING_DOCS | Worktree |
| `cai-review-pr` | Pre-merge ripple-effect review — finds inconsistencies the PR introduced but didn't update, and raises out-of-scope issues for problems outside the PR's authorized scope | Read, Grep, Glob | haiku | PR state REVIEWING_CODE | Worktree |
...
| `cai-triage` | INTERNAL — Triage `auto-improve:raised` issues one at a time — classify as REFINE, PLAN_APPROVE, APPLY, or HUMAN. Inline-only — full issue body is provided in the user message. Minimal tool use. | (none) | haiku | Issue states RAISED / TRIAGING | Inline-only |
...
**Inline-only** agents receive all context in the user message and require no file access. **Worktree** agents run in a fresh git clone provided by the wrapper; code-editing agents (`cai-implement`, `cai-revise`, `cai-rebase`) commit changes and open PRs, while review/planning agents (`cai-code-audit`, `cai-git`, `cai-plan`, `cai-propose`, `cai-propose-review`, `cai-review-docs`, `cai-review-pr`, `cai-update-check`) read from the clone and emit structured output. **Clone** agents (`cai-explore`) also run against a fresh repo clone but post outcomes directly to GitHub issues rather than opening PRs. **Read-only** agents read the repo or external data without writing anything.
