...
manually or from CI. `scripts/generate-fsm-docs.py` regenerates
...
- Writes **docs** — `generate-fsm-docs.py` owns `docs/fsm.md`.
- Run by **actions** — the `REVIEWING_DOCS` FSM handler
  (`cai_lib/actions/review_docs.py`) invokes the generator and
...
## Operational notes
- **Generator invariants.** `docs/fsm.md` is generated and should not be
  hand-edited; it is pure-render. PRs that
  hand-edit this file will see their changes overwritten by
...
- **CI implications.** The `REVIEWING_DOCS` FSM handler depends
  on the generator staying idempotent and on
