...
- [`cai_lib/subagent/__init__.py`](../../cai_lib/subagent/__init__.py) — Base classes `SubAgent` and `CostTracker` (repo-agnostic, zero cai-specific logic); re-exports are now in [`cai_lib/cai_subagent.py`](../../cai_lib/cai_subagent.py).
- [`cai_lib/subagent/core.py`](../../cai_lib/subagent/core.py) — `SubAgent` class drives agent execution via the Claude Agent SDK with typed options; `SubAgent.run(prompt)` returns a typed `RunResult` (replacing the legacy `subprocess.CompletedProcess` shape) so callers can inspect structured outcomes without re-parsing opaque stdout/stderr strings (issue #1277). Owns the `_collect_results` query driver and the `cli_path` pin that keeps the SDK pointed at the npm-installed `claude` binary (issue #1226).
...
- [`cai_lib/claude_argv.py`](../../cai_lib/claude_argv.py) — Deprecated `_run_claude_p(...)` argv facade wrapping the SDK with `claude -p` command-line syntax; `_argv_to_options(argv, cwd)` parses command-line arguments into SDK options; both retained for 12+ remaining handlers not yet ported to `run_subagent`; relocated from `cai_lib/subagent/legacy.py` in this PR for top-level visibility.
