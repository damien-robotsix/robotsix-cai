...
| `cai.py verify` / `audit` / `unblock` | _(own cron schedules; also manual/on-demand)_ | Housekeeping subcommands that are not FSM handlers. Most per-state handlers (triage, refine, plan, implement, explore, confirm, maintain, review-pr, revise, fix-ci, merge) are no longer standalone subcommands — invoke them via `cai.py dispatch`. Exception: `cai.py review-docs --pr N` is a standalone CI-mode entry point (invoked by `regenerate-docs.yml` to retry docs review with module-coverage auto-fix). |
