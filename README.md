...
| `cai.py fix` | `15 * * * *` (hourly :15) | Scores eligible issues by age, category success rate, and prior fix attempts; picks the highest scorer, first runs a cheap Haiku pre-screen to classify the issue; spike/ambiguous issues are returned to their origin label without cloning; if actionable, runs 2 parallel plan agents then a select agent to choose the best plan, lets a fix subagent implement it with full tool permissions, opens a PR — see lifecycle below |
