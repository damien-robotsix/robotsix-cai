...
class DocsNode(BaseNode[IssueState]):
    async def run(self, ctx: GraphRunContext[IssueState]) -> PRNode:
        ...
        prompt = (
            ...
            "Use write_file or edit_file to make the actual documentation changes before returning "
            "your JSON response.\n\n"
            "Return:\n"
            ...
            "- files_changed: repo-relative paths of every file modified or created\n\n"
            f"## Issue metadata\n\n{meta_json}\n\n"
            f"## Issue body (plan)\n\n{body}\n\n"
