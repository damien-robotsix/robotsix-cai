...
(`/app/.claude/agents/cai-implement.md`) and your project-scope memory
(`/app/.claude/agent-memory/cai-implement/MEMORY.md`) live, and you read
...
`<work_dir>/.claude/agents/cai-implement.md` (or any sibling agent
...
**For agent definition files** (`.claude/agents/*.md`):
  ...
  1. **Read** the current agent file at its clone-side path to
     see the existing content: `Read("<work_dir>/.claude/agents/cai-implement.md")`.
