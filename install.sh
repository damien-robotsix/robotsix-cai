...
#
# Update shell aliases only (no reinstall, no volume wipe):
#
#   bash install.sh --aliases-only
#   INSTALL_DIR=/path/to/robotsix-cai bash install.sh --aliases-only
#
# Requires INSTALL_DIR to point at a prior install (containing
# docker-compose.yml); no other side effects.
#
...
# Parse the single optional mode flag. Default mode runs the full install;
# --aliases-only (also accepted as the bare subcommand `aliases`) jumps
# straight to the alias-configuration prompt, skipping volume wipes,
# image pulls, and all interactive auth steps.
MODE="install"
if [[ $# -gt 0 ]]; then
  case "$1" in
    ...
    *)
      ...
      echo "Usage: bash install.sh [--aliases-only]" >&2
      exit 1
      ;;
  esac
fi
...
# Prompt for and write the shell-alias block. Requires $DC to be set by
# the caller (docker compose command prefix). Shared between the full
# install path and the --aliases-only short-circuit; behaviour must stay
# byte-identical between both invocations.
configure_aliases() {
  echo
...
# --aliases-only short-circuit: validate that a prior install exists,
# wire DC, run the alias prompt, and exit. Everything below this point
# (installer banner, auth prompts, compose generation, volume wipe,
# image pull, gh/git/claude auth, "Next steps" banner) is reserved for
# the full-install path and must stay untouched.
if [[ "$MODE" == "aliases-only" ]]; then
  if [[ ! -f "$INSTALL_DIR/docker-compose.yml" ]]; then
    echo "ERROR: no docker-compose.yml found at $INSTALL_DIR" >&2
    echo "       --aliases-only requires a prior install. Run it as:" >&2
    echo "         INSTALL_DIR=/path/to/robotsix-cai bash install.sh --aliases-only" >&2
    exit 1
  fi
...
case "$AUTH_CHOICE" in
  1)
    AUTH_COMMENT="# Auth mode: in-container claude REPL OAuth (credentials in cai_home volume)"
    ENV_FILE_BLOCK=""
    HOME_VOLUME_COMMENT=$(cat <<'EOF'
...
EOF
  ...
  2)
    ...
    AUTH_COMMENT="# Auth mode: API key from .env"
