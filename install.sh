...
echo
echo "  1) Run 'claude auth login' inside the container after install"
echo "     (recommended; OAuth credentials land in the cai_claude"
echo "     volume and persist across restarts; no static secret"
echo "     stored in the container env. The installer runs the login"
echo "     step for you automatically.)"
echo
...
# Auth mode: in-container claude auth login (OAuth in cai_claude volume)
services:
  cai:
    ...
    volumes:
      # Persistent claude-code state (credentials + transcripts).
...
services:
  cai:
    ...
    environment:
      ...
      CAI_TRANSCRIPT_MAX_FILES: "50"        # read at most N recent transcript files (0 = no limit)
    volumes:
      # Persistent claude-code state (transcripts + any cached
      # config). API-key auth means the volume can stay empty for
      # credentials, but transcripts still accumulate here under
      # `projects/`.
      - cai_claude:/home/cai/.claude
      # Persistent per-agent memory (\`.claude/agent-memory/<name>/\`)
      # so the durable notes each subagent accumulates across runs
...
if [[ "$TTY" == "/dev/tty" ]]; then
  ...
  # interactive login needed. claude auth login opens a device-code
  # flow; the user pastes the code into a browser to complete it.
  # Credentials persist in the cai_claude volume.
  if [[ "$AUTH_CHOICE" == "1" ]]; then
    echo
    echo "Authenticating claude inside the container (interactive)."
    echo "Credentials persist in a Docker volume named 'cai_claude' so"
    echo "subsequent 'docker compose up' runs don't need to re-authenticate."
    echo
    if ! docker compose run --rm cai claude auth login < /dev/tty; then
      echo
      echo "[!] claude auth login did not complete. Rerun it yourself:"
      echo "      cd $INSTALL_DIR && docker compose run --rm cai claude auth login"
      exit 1
    fi
    echo
    echo "[OK] claude is authenticated. Credentials persisted in cai_claude."
  fi
  ...
  if [[ "$AUTH_CHOICE" == "1" ]]; then
    echo "      docker compose run --rm cai claude auth login"
  fi
