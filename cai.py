...
_AUDIT_KIND_CHOICES = ("good-practices", "code-reduction", "cost-reduction", "workflow-enhancement")


def _build_parser() -> argparse.ArgumentParser:
    """Build the top-level argparse parser.

    Extracted from ``main()`` so unit tests can exercise the
    subcommand surface without triggering the gh/claude auth
    side-effects or the handler dispatch.
    """
    ...
    # Canonical audit entry point: ``cai audit <kind>``. Nested
    # subparser picks between the four per-module audit kinds
    # (good-practices, code-reduction, cost-reduction,
    # workflow-enhancement) and the structured-log health monitor
    # (health). ``audit-module --kind <kind>`` and ``audit-health``
    # are kept as hidden back-compat aliases so stale shell
    # aliases generated by earlier installers keep working.
    audit_p = sub.add_parser(
        "audit",
        help=(
            "On-demand audit entry point. Pick a kind: "
            "good-practices | code-reduction | cost-reduction | "
            "workflow-enhancement | health."
        ),
    )
    audit_sub = audit_p.add_subparsers(dest="audit_kind", required=True)
    for _kind in _AUDIT_KIND_CHOICES:
        audit_sub.add_parser(
            _kind,
            help=(
                f"Per-module {_kind} audit: iterate every module in "
    ...
    audit_sub.add_parser(
        "health",
        help=(
            "Audit-health monitor: reads /var/log/cai/audit/*/*.jsonl "
            "for the last 30 days and raises findings for error "
            "conditions, stale audits, cost anomalies, and degenerate "
            "zero-findings runs."
        ),
    )

    # Hidden back-compat aliases. ``cai audit-module --kind <kind>``
    # and ``cai audit-health`` still parse so pre-existing shell
    # aliases / scripts do not break; they route to the same
    # handlers as ``cai audit <kind>`` / ``cai audit health``.
    audit_module_p = sub.add_parser("audit-module", help=argparse.SUPPRESS)
    audit_module_p.add_argument(
        "--kind",
        required=True,
        choices=list(_AUDIT_KIND_CHOICES),
        help="Per-module audit kind to dispatch",
    )
    sub.add_parser("audit-health", help=argparse.SUPPRESS)
...
def _dispatch_audit(args) -> int:
    """Route the canonical ``cai audit <kind>`` invocation.

    ``health`` goes to :func:`cmd_audit_health`; the four per-module
    kinds go to :func:`cmd_audit_module` after aliasing
    ``args.audit_kind`` onto ``args.kind`` so the shared handler
    keeps its original attribute contract.
    """
    if args.audit_kind == "health":
        return cmd_audit_health(args)
    args.kind = args.audit_kind
    return cmd_audit_module(args)
